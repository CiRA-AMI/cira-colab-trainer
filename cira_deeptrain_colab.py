...
def onGenerateClicked(p):
    ...
    subprocess.call(
        [
            "bash",
            "-c",
            "source /root/install/setup.bash && export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/lib && /root/install/lib/cira_colab_deeptrain/cira_colab_deeptrain_run gen --platform offscreen",
        ]
    )
    timer.cancel()
    jso = readLogGen()
...
def onTrainClicked(p):
    ...
    subprocess.Popen(
        [
            "bash",
            "-c",
            "source /root/install/setup.bash && export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/lib && /root/install/lib/cira_colab_deeptrain/cira_colab_deeptrain_run train --platform offscreen",
        ]
    )
